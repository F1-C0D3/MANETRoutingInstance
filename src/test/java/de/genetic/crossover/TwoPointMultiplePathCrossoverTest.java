...
public class TwoPointMultiplePathCrossoverTest {
	...
	public void directeConnectedSourceTargetPathSingleFlowTest()
		...
		for (int i = 0; i < 100; i++) {
			new TwoPointMultiplePathCrossover().recombine(Arrays.asList(genomeOne, genomeTwo),
...
//				new LinkUtilizationPrinter<ScalarRadioLink, ScalarLinkQuality>()));
		...
		for (int i = 0; i < 100; i++) {
			...
			testGenome.add(genome.createRandomInstance(randomInstance.getDoubleRandom()));
			GraphGenome result = new TwoPointMultiplePathCrossover().recombine(testGenome,
					randomInstance.getDoubleRandom());
